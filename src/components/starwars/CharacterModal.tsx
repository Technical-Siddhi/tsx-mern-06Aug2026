    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          {/* Backdrop Blur & Click Outside Overlay */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-slate-900 border border-amber-500/30 rounded-3xl shadow-2xl overflow-hidden z-10 backdrop-blur-2xl text-slate-100"
          >
            {/* Modal Header Bar with Close Button */}
            <div className="absolute top-4 right-4 z-20">
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full bg-slate-950/80 border border-slate-800 text-slate-300 hover:text-white hover:border-amber-400 flex items-center justify-center transition shadow-lg"
            ...
            <div className="p-6 sm:p-8 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
              {/* Physical Attributes & Demographics Grid */}
              <div>
                <h4 className="text-xs font-mono font-bold uppercase text-amber-400 mb-3 tracking-wider">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
                  ...
                  <div className="p-3 rounded-2xl bg-slate-950/60 border border-slate-800">
                    ...
                    <span className="text-slate-100 font-bold text-sm">{character.birthYear}</span>
                  </div>
                  <div className="p-3 rounded-2xl bg-slate-950/60 border border-slate-800">
                    <span className="block text-slate-500 text-[10px] uppercase">Created Date</span>
              ...
              <div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-2 font-mono text-xs">
            ...
            <div className="p-4 bg-slate-950 border-t border-slate-800 flex justify-end">
              <button
                onClick={onClose}
                className="px-5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-mono text-xs font-bold transition"
              >
                Close Holocron
              </button>
            </div>
